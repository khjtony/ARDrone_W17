...
def gopro_live():
	...
	response = urllib.urlopen('http://10.5.5.9/gp/gpControl/info').read()
	if b"HD4" in response or b"HD3.2" in response or b"HD5" in response or b"HX" in response:
		...
		if b"HX" in response:
			...
			while connectedStatus == False:
				req=urllib.urlopen("http://10.5.5.9/gp/gpControl/status")
				data = req.read()
		...
		response = urllib.urlopen('http://10.5.5.9/camera/cv').read()
		if b"Hero3" in response:
			PASSWORD=urllib.urlopen("http://10.5.5.9/bacpac/sd").read()
			print("HERO3/3+/2 camera")
			Password =  str(PASSWORD, 'utf-8')
			text=re.sub(r'\W+', '', Password)
			urllib.urlopen("http://10.5.5.9/camera/PV?t=" + text + "&p=%02")
...
def quit_gopro(signal, frame):
	if RECORD:
		urllib.urlopen("http://10.5.5.9/gp/gpControl/command/shutter?p=0").read()
	sys.exit(0)
